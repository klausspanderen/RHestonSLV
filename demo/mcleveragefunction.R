...
params <- HestonSLVMCParams(TRUE, 90, 100, 40000)
...
times <- seq(0.01, 5, 0.1)
strikes <- seq(2.75, 6.5, 0.1)

g <- expand.grid(strikes=strikes, times=times)

g$vol <- unlist(as.list(
  sapply(times, function(time) {
    sapply(strikes, function(strike) { leverageValue(mcModel, time, exp(strike), FALSE) })
  }) ))
...
wireframe(vol ~ strikes*times, g,
          ...
          zoom=0.95,
          main=list(label=paste("Monte-Carlo Simulation, \U03B7=",sprintf("%3.2f",1.0), sep=""),
                    cex=2.0, font=1),
          zlim=c(0.25, 2.75),
